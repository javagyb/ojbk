package me.javagyb.data.supplier.commons;
...
public abstract class ClassUtils {
	...
	public static Set<Class<?>> getClassesByPackageName(String pack) {
	...
	/**
	 * 鑾峰彇绫籧lazz鐨勬墍鏈塅ield锛屽寘鎷叾鐖剁被鐨凢ield锛屽鏋滈噸鍚嶏紝浠ュ瓙绫籉ield涓哄噯銆�
	 * @param clazz
	 * @return Field鏁扮粍
	 */
	...
	/**妫�祴Field List涓槸鍚﹀凡缁忓寘鍚簡鐩爣field
	 * @param fieldList
	 * @param field 甯︽娴媐ield
	 * @return
	 */
	...
	public static boolean isAnnotation(Method method,Class<? extends Annotation> annotation){
		...
		for (Annotation current : declaredAnnotations) {
	...
	public static <K extends Annotation> K find(Field field,Class<K> clazz){
	...
	public static int index(Field field,Class<? extends  Annotation> clazz){
		Annotation[] declaredAnnotations = field.getDeclaredAnnotations();
		for (int i=0;i<declaredAnnotations.length;i++){
			if(clazz.isAssignableFrom( declaredAnnotations[i].annotationType())){
				return i;
			}
		}
		return -1;
	...
	public static List<Annotation> exclude(Field field, Class<? extends  Annotation> clazz){
		Annotation[] declaredAnnotations = field.getDeclaredAnnotations();
		List<Annotation> temp = new ArrayList<>();
		for (int i=0;i<declaredAnnotations.length;i++){
			if(!clazz.isAssignableFrom( declaredAnnotations[i].annotationType())){
				temp.add(declaredAnnotations[i]);
			}
		}
		return temp;
	}

	public static <K extends Annotation> K find(Class clazz,Class<K> anno){
		for (Annotation annotation : clazz.getDeclaredAnnotations()) {
			if(anno.isAssignableFrom( annotation.annotationType())){
				return (K) annotation;
			}
		}
		return null;
	...
	public static <K extends Annotation> K find(Method method,Class<K> clazz){
	...
	public static <K extends Annotation> K find(Annotation[] annotaions,Class<K> clazz){
	...
	public static <K extends Annotation> K find(Annotation[] annotaions,Class<K>... clazzes){
		for(Class<?> current :clazzes ){
			for (Annotation annotation : annotaions) {
				if(current.isAssignableFrom( annotation.annotationType())){
					return (K) annotation;
				}
			}
		}
	...
	public static <K extends Annotation> K find(Annotation[] annotaions, Set<Class<? extends Annotation>> clazzes){
		for(Class<?> current :clazzes ){
			for (Annotation annotation : annotaions) {
				if(current.isAssignableFrom( annotation.annotationType())){
					return (K) annotation;
	...
	public static boolean isCollection(Class<?> c){ return Collection.class.isAssignableFrom(c);}
